...
export const CategorySelector: React.FC<CategorySelectorProps> = ({
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* Category Selector - Desktop: inline list, Mobile: full-screen modal */}
      <div>
        {/* Selected Category Display / Toggle */}
        <ThemedCard
          hover
          padding="sm"
        ...
        >
          {selectedCategory ? (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Brain className="w-4 h-4" style={{ color: 'var(--color-text-tertiary)' }} />
            ...
            <div className="max-h-56 overflow-y-auto">
              {filteredCategories.length > 0 ? (
                ...
                <div className="px-4 py-6 text-center text-gray-500">
                  <Brain className="w-6 h-6 mx-auto mb-1.5 text-gray-400" />
                  <p className="text-sm">No categories found{searchTerm ? ` matching "${searchTerm}"` : ''}</p>
                </div>
              )}
            </div>
          </div>
