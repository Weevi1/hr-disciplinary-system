...
export const EmployeeFormModal: React.FC<EmployeeFormModalProps> = ({
  ...
  const [formData, setFormData] = useState<EmployeeFormData>({
    ...
    probationEndDate: '',
    managerIds: [], // 🔧 UPDATED: Multi-manager support
    managerId: '', // ⚠️ DEPRECATED: Keep for backward compatibility
    isActive: true
  });
  ...
  useEffect(() => {
    if (employee) {
      ...
      setFormData(prev => ({
        ...
        managerIds: user?.id ? [user.id] : [], // 🔧 UPDATED: Assign to current user as manager
        managerId: user?.id || '', // ⚠️ DEPRECATED: Keep for backward compatibility
        isActive: true
      }));
    }
      ...
      if (employee) {
        ...
        const updatedEmployee = {
          ...
          profile: {
            ...
            email: formData.email || '', // 🔧 FIXED: Handle optional email
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className={basicMode ? "md:col-span-1" : "md:col-span-2"}>
                  ...
                  {loadingManagers ? (
                    ...
                    <div className="text-sm text-slate-500 p-3 border border-slate-200 rounded-lg">
                      No managers available in organization
                    </div>
                  ) : (
                    <div className="border border-slate-200 rounded-lg max-h-48 overflow-y-auto">
                      {managers.map(manager => {
                        const isChecked = formData.managerIds?.includes(manager.id) || false;
                        return (
                          <label
                            key={manager.id}
                          ...
                          >
                            <input
                              ...
                              checked={isChecked}
                              onChange={(e) => {
                                const currentIds = formData.managerIds || [];
                                const newIds = e.target.checked
                                  ? [...currentIds, manager.id]
                                  : currentIds.filter(id => id !== manager.id);
                                setFormData({
                                  ...formData,
                                  managerIds: newIds,
                                  managerId: newIds[0] || '' // Keep first for backward compatibility
                                });
                              }}
