...
import { generateWarningPDF_v1_2_0 as _generateWarningPDF_v1_2_0_Impl } from './pdf/versions/v1_2_0';
...
export class PDFGenerationService {
  ...
  /**
   * 🆕 VERSION 1.2.0 — CURRENT delegate. Implementation moved verbatim to
   * `pdf/versions/v1_2_0.ts` in Phase 2 Tier 3B step 7. Body is byte-identical
   * to the original; `this.X` helper refs swapped for direct imports.
   * (Pre-existing bug: `addAppealReportSection` reference preserved via
   *  local stub — see the new module's file header for context.)
   */
  private static async generateWarningPDF_v1_2_0(data: WarningPDFData, customSettings?: any): Promise<Blob> {
    return _generateWarningPDF_v1_2_0_Impl(data, customSettings);
  }
