  ...
  async create(warningData: EnhancedWarningFormData): Promise<string> {
    try {
      ...
      const warning: Warning = {
        id: warningData.id || '', // Use existing ID if updating, empty if creating (Firestore will generate)
...
export const recognitions = {
  /**
   * Get all recognitions for an organization
   */
  async getAll(organizationId: string, filters?: {
    employeeId?: string;
    ...
    try {
      ...
      const recognitionsRef = collection(db, 'organizations', organizationId, 'recognitions');
      ...
      Logger.success(`Loaded ${recognitions.length} recognitions`);
      return recognitions;
    } catch (error) {
      handleError('recognitions.getAll', error);
    }
  },
   ...
   */
  async getById(organizationId: string, recognitionId: string): Promise<any> {
    try {
      const { doc, getDoc } = await import('firebase/firestore');
      const recognitionRef = doc(db, 'organizations', organizationId, 'recognitions', recognitionId);
      const snapshot = await getDoc(recognitionRef);
      ...
      handleError('recognitions.getById', error);
    }
  },
   ...
   */
  async create(organizationId: string, recognitionData: any): Promise<string> {
    try {
      ...
      const docRef = await addDoc(recognitionsRef, cleanedData);

      Logger.success('Recognition created successfully', { id: docRef.id });
      return docRef.id;
    } catch (error) {
      handleError('recognitions.create', error);
    }
  },
   ...
   * Update recognition
   */
  async update(organizationId: string, recognitionId: string, updates: any): Promise<void> {
    try {
      ...
      const recognitionRef = doc(db, 'organizations', organizationId, 'recognitions', recognitionId);
      await updateDoc(recognitionRef, cleanedUpdates);

      Logger.success('Recognition updated successfully', { id: recognitionId });
    } catch (error) {
      handleError('recognitions.update', error);
    }
  },
   ...
   * Delete recognition
   */
  async delete(organizationId: string, recognitionId: string): Promise<void> {
    try {
      const { doc, deleteDoc } = await import('firebase/firestore');
      const recognitionRef = doc(db, 'organizations', organizationId, 'recognitions', recognitionId);
      await deleteDoc(recognitionRef);

      Logger.success('Recognition deleted successfully', { id: recognitionId });
    } catch (error) {
      handleError('recognitions.delete', error);
    }
  },

  /**
   * Generate certificate for recognition
   */
  async generateCertificate(organizationId: string, recognitionId: string): Promise<string> {
    try {
      // Import PDFGenerationService (will be implemented separately)
      // For now, return placeholder
      throw new Error('Certificate generation not yet implemented');
    } catch (error) {
      handleError('recognitions.generateCertificate', error);
    }
  }
};
